...
function enviarCorreoAprobacion($datos, $rol_aprobacion) {
    try {
        ...
        $mail->setFrom(getenv('MAIL_FROM'), 'Sistema de Comedor - BacroCorp');
        $mail->addAddress($correo_destino, $datos['NOMBRE']);
        $mail->addReplyTo(getenv('MAIL_REPLYTO'), 'Coordinación de Comedor');
        $mail->addCC(getenv('MAIL_FROM'), 'Administración TI');
